...
const T = {
  ...
  st_thumb_p: { en:"1280×720, the size YouTube asks for. Drawn on this device — nothing is uploaded and nothing is generated by a model.", zh:"1280×720，YouTube 要求的尺寸。在本机绘制——什么都不上传，也不用模型生成。", hi:"1280×720, वही साइज़ जो YouTube माँगता है। इसी डिवाइस पर बनता है — कुछ भी अपलोड नहीं होता और कोई मॉडल इसे नहीं बनाता।", es:"1280×720, el tamaño que pide YouTube. Se dibuja en este dispositivo: no se sube nada ni lo genera un modelo.", ar:"‎1280×720، المقاس الذي يطلبه يوتيوب. يُرسم على هذا الجهاز — لا يُرفع شيء ولا يولّده نموذج.", fr:"1280×720, la taille demandée par YouTube. Dessiné sur cet appareil : rien n’est envoyé et rien n’est généré par un modèle.", bn:"১২৮০×৭২০, YouTube যে মাপ চায়। এই ডিভাইসেই আঁকা হয় — কিছুই আপলোড হয় না, কোনো মডেলও বানায় না।", pt:"1280×720, o tamanho que o YouTube pede. Desenhado neste dispositivo — nada é enviado e nada é gerado por um modelo.", ru:"1280×720 — размер, который просит YouTube. Рисуется на этом устройстве: ничего не загружается и ничего не генерирует модель.", ur:"‎1280×720، وہی سائز جو یوٹیوب مانگتا ہے۔ اسی ڈیوائس پر بنتا ہے — کچھ اپ لوڈ نہیں ہوتا اور نہ کوئی ماڈل بناتا ہے۔", id:"1280×720, ukuran yang diminta YouTube. Digambar di perangkat ini — tidak ada yang diunggah dan tidak ada model yang membuatnya.", de:"1280×720, die Größe, die YouTube verlangt. Auf diesem Gerät gezeichnet — nichts wird hochgeladen und nichts von einem Modell erzeugt.", ja:"1280×720、YouTubeが求めるサイズです。この端末で描画します。何もアップロードせず、モデルも使いません。", tr:"1280×720, YouTube’un istediği boyut. Bu cihazda çizilir — hiçbir şey yüklenmez ve bir model üretmez.", ko:"1280×720, 유튜브가 요구하는 크기입니다. 이 기기에서 그립니다 — 업로드도 없고 모델이 만들지도 않습니다.", fa:"‎۱۲۸۰×۷۲۰، همان اندازه‌ای که یوتیوب می‌خواهد. روی همین دستگاه کشیده می‌شود — چیزی آپلود نمی‌شود و مدلی آن را نمی‌سازد.", uk:"1280×720 — розмір, який просить YouTube. Малюється на цьому пристрої: нічого не завантажується й нічого не генерує модель.", it:"1280×720, la misura che chiede YouTube. Disegnata su questo dispositivo: non si carica nulla e non la genera un modello.", pl:"1280×720, rozmiar, którego wymaga YouTube. Rysowane na tym urządzeniu — nic nie jest wysyłane ani generowane przez model.", vi:"1280×720, kích thước YouTube yêu cầu. Vẽ ngay trên thiết bị này — không tải gì lên và không do mô hình tạo ra." },
...
/* ============================================================
   ...
   exactly as it does now, with every save staying in this browser.
   ============================================================ */
...
/* ============================================================================
   ...
   There is one route to a model, and no key ever reaches a browser:

     THE SITE WORKER   the key lives on the Cloudflare Worker, not in the page,
                       which is the only correct place for it — anything shipped
                       to a browser is public, and a Gemini key in a static file
                       is a key someone else is spending.

   Behind that one address the worker keeps three vendors in turn, so a quota
   wall at one of them is a switch rather than an outage. A model running on the
   visitor's own machine (Ollama) is the only other route, and it is opt-in.

   There used to be a third: a personal Google key pasted into the profile and
   sent straight from the page. It is gone. It asked a fourteen-year-old to hold
   a billable credential in localStorage, on a site whose whole safety story is
   that nothing of theirs is anywhere it should not be.
   ============================================================================ */
...
/* One visitor's leftovers from when the profile had a key box. Cleared on
   sight rather than left to sit in localStorage on every device that ever
   used the old build, because the box that could delete it is gone. */
try { localStorage.removeItem('nc_ai_key'); } catch (e) {}
...
/* Returns { text, image, err }. It never throws and it never returns a made-up
   ...
   the active selection (ncActiveProvider); model defaults per provider. Every
   vendor is reached through the worker, which holds the keys. search:true
...
function ncSayWhy(reason, status) {
  ...
  if (/from this connection|too many requests/i.test(r))
    return 'NovaClip is limiting how fast this internet connection can ask — 20 questions a minute, ' +
           'shared by everyone on the same wifi. Wait a minute and try again.';
  if (/quota|rate limit|resource has been exhausted/i.test(r))
    return 'NovaClip\'s shared AI is out of free requests for now. The quota refills, so ' +
           'come back in a few minutes.';
  if (/api key not valid|invalid api key|api_key_invalid/i.test(r))
    return 'The key this site uses was rejected by Google. Whoever deployed the worker ' +
           'needs to replace it — nothing on your side can fix this one.';
...
/* The routes this request may take, best first, cooled-down ones removed.
   ...
   aimed at the reader. */
function ncRoutes(provider, model, opts) {
...
async function ncAsk(prompt, opts) {
...

  {
    ...
    /* ------------------------------------------------------------------
      ...
      if (r.ok) {
        ...
        if (route.kind === 'worker') ncModelWorks(route.provider, route.model);
      ...
      /* THE NAME IS DEAD, NOT THE REQUEST.
         The worker walks its own copy of NC_MODELS before it answers, so by
         the time a "no such model" reaches here every name has already been
         tried and retrying from this side would just spend the same calls
         twice. What is left to do is say which file has the dead string in it,
         because that is the one thing the status code never tells anybody. */
      const gone = ncModelGone(r.status, reason);

      let why = '';
      if (gone) {
        why = 'The AI model names this site uses are not being served any more (asked for ' +
              route.model + '). They need updating in nova.js and on the worker.';
      }
    ...
    if (!via) {
      ...
      if (tried.length > 1) {
        ...
        err += ' Every AI NovaClip could reach was asked (' + tried.length + ' of them) and ' +
               'none of them could answer. Give it a few minutes — this is almost always ' +
  ...
  /* Who answered in the end, and whether that was the first choice. A feature
     that wants to say "the usual AI was busy, this came from somewhere else"
